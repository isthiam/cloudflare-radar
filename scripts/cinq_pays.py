...
DEFAULT_N_MIN = 5
DEFAULT_N_MAX = 8  # car 2^8=256 < 365
R_USED = 1
...
def hill_alpha_hat(increments: np.ndarray, k=None) -> float:
...
for c, sub in df.groupby("country_iso3"):
    x_level = sub["attack_ipv6_pct"].astype(float).values
    dx = np.diff(x_level)
    ...
    if not np.isfinite(p) or n_max < n_min or n_max < 5:
        results.append({"country": c, "n_obs": len(dx), "alpha": alpha, "p_used": p,
                        "H_median": np.nan, "note":"insufficient_scales_or_bad_alpha"})
        continue

    rows = hurst_detail(dx, p=p, r=R_USED, n_min=n_min, n_max=n_max)
    if not rows:
        results.append({"country": c, "n_obs": len(dx), "alpha": alpha, "p_used": p,
                        "H_median": np.nan, "note":"no_valid_scales"})
        continue
    ...
    for (n,h,v0,vr) in rows:
        details.append({"country": c, "n": n, "H": h, "V0": v0, "Vr": vr, "alpha": alpha, "p_used": p})

res_df = pd.DataFrame(results).sort_values("country")
det_df = pd.DataFrame(details).sort_values(["country","n"])

res_df.to_csv("hurst_results_daily_by_country.csv", index=False, encoding="utf-8")
det_df.to_csv("hurst_detail_daily_by_country.csv", index=False, encoding="utf-8")

print("\n=== Résultats DAILY (attaque_ipv6_pct) ===")
print(res_df.to_string(index=False))
print("\nExports: hurst_results_daily_by_country.csv, hurst_detail_daily_by_country.csv")
