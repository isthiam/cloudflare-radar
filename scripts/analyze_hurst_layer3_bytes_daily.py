...
# PARAMÈTRES
# ======================
CSV = "radar_attacks_l3_bytes_daily_by_country.csv"

DEFAULT_N_MIN = 5
DEFAULT_N_MAX = 8        # 2^8 = 256 < 365
R_USED = 1

MIN_NONZERO = 120        # un peu plus strict, bytes/log -> mieux
MAX_P_CAP = 1.5          # p max (stabilité)
...
def choose_p(alpha: float) -> float:
    """
    Choix prudent: p < alpha.
    Comme on est sur log1p(bytes), p peut aller un peu au-dessus de 1.
    """
...
for country, sub in df.groupby("country"):
    ...
    bytes_series = sub["attack_bytes"].astype(float).values
    y = np.log1p(bytes_series)
    ...
    n_min = min(DEFAULT_N_MIN, n_max_possible)
    n_max = min(DEFAULT_N_MAX, n_max_possible)
...
res_df.to_csv("hurst_results_layer3_bytes_daily.csv", index=False, encoding="utf-8")
det_df.to_csv("hurst_detail_layer3_bytes_daily.csv", index=False, encoding="utf-8")

print("\n=== Résultats DAILY – Layer3 bytes (log1p) ===")
print(res_df.to_string(index=False))
print("\nExports:")
print(" - hurst_results_layer3_bytes_daily.csv")
print(" - hurst_detail_layer3_bytes_daily.csv")
